...
mcmc_spline <- function(nknot = 5) {
  ...
  p_t <- function(sig_sq, beta) {
    (-1/(2*sig_sq)) * t(y - (X %*% beta)) %*% (y - (X %*% beta))
  }
  ...
  met_gibbs <- function(its) {
    ...
    ar <- 0
    for(it in 2:its) {
      propose_t <- function(x, prev, curr) {
        log(dunif(x, min = prev, max = curr))
      }
    
      candidate_t <- runif(5, mat_t[its-1,]-1, mat_t[its-1,]+1)
...
burn <- 1:15
bayesian <- bayes_spline(seq(0,1,by=.1), y = y, x = x)

mat_beta <- bayesian[[1]][-burn,]
mat_sig <- bayesian[[2]][-burn]
X <- bayesian[[3]]

colMeans(mat_beta) #target: -1.83, 25.19, -91.23, 135.03
mean(mat_sig) #target: 1.46
