...
bass<-function(X,y,max.int=3,max.basis=50,tau2=10^4,nu=30,nmcmc=10000,g1=1,g2=1,h1=10,h2=10){
  Xt<-t(X)
  n<-length(y)
  p<-ncol(X)
  ssy<-sum(y^2)
  ...
  Vinv.curr<-t(X.curr)%*%diag(1/v[1,])%*%X.curr+1/tau2 
  bhat<-solve(Vinv.curr)%*%t(X.curr)%*%diag(1/v[1,])%*%y
  d.curr <- g2 + t(y)%*%diag(1/v[1,])%*%y - crossprod(bhat,Vinv.curr%*%bhat)
  ...
  for(i in 2:nmcmc){
    ...
    if(move.type=='birth'){
      ...
      Vinv.cand<-t(X.cand)%*%diag(1/v[i-1,])%*%X.cand+diag(nbasis[i-1]+2)/tau2 # +2: one for intercept and one for birth
      bhat.cand<-solve(Vinv.cand)%*%t(X.cand)%*%diag(1/v[i-1,])%*%y
      d.cand <- t(y)%*%diag(1/v[i-1,])%*%y - t(bhat.cand)%*%Vinv.cand%*%bhat.cand
      ...
      Vinv.cand<-t(X.cand)%*%diag(1/v[i-1,])%*%X.cand+diag(nbasis[i-1])/tau2
      bhat.cand<-solve(Vinv.cand)%*%t(X.cand)%*%diag(1/v[i-1,])%*%y
      d.cand <- t(y)%*%diag(1/v[i-1,])%*%y - t(bhat.cand)%*%Vinv.cand%*%bhat.cand
      ...
      Vinv.cand<-t(X.cand)%*%diag(1/v[i-1,])%*%X.cand+diag(nbasis[i-1]+1)/tau2
      bhat.cand<-solve(Vinv.cand)%*%t(X.cand)%*%diag(1/v[i-1,])%*%y
      d.cand <- t(y)%*%diag(1/v[i-1,])%*%y - t(bhat.cand)%*%Vinv.cand%*%bhat.cand
    ...
    S<-solve(t(X.curr)%*%diag(1/v[i-1,])%*%X.curr+diag(nbasis[i]+1)/tau2)
    beta[i,1:(nbasis[i]+1)]<-rmnorm(1,S%*%t(X.curr)%*%diag(1/v[i-1,])%*%y,S)
    v[i,]<-1/rgamma(n,(nu+1)/2,.5*(s2[i-1]*nu + (y-X.curr%*%t(beta[i,1:(nbasis[i]+1),drop=F]))^2))
    s2[i]<-rgamma(1,n*nu/2+g1,rate=g2+nu/2*sum(1/v[i,]))
  }
