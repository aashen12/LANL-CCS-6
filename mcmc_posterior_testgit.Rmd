...
system.time(p_sig(ST = sig, sig_sq = 1, .mu = c(0,0,0)))
system.time(p_sig2(ST = sig, sig_sq = 1, .mu = c(0,0,0)))
```
...
met_gibbs <- function(its) {
  ...
  propose_sig <- function(x, mn, sd = 0.1) {
    #browser()
    log(dtruncnorm(x, a = 0, mean = mn, sd = sd))
  }
  
  propose_rho <- function(x, mn, sd = 0.8) {
    log(dtruncnorm(x, a = -1, b = 1, mean = mn, sd = sd))
  }
  ...
  for(i in 2:its) {
    ...
    cor_to_cov <- c(
      (mat_sig[i-1,1] * mat_sig[i-1,2]),
      (mat_sig[i-1,1] * mat_sig[i-1,3]),
      (mat_sig[i-1,2] * mat_sig[i-1,3])
    ...
    for(k in 1:3) {
      ...
      1, a = -1, b = 1, mean = met_mu[i,k], sd = 0.09
      )
      ...
      (p_rho(rho = candidate_rho, .mu = met_mu[i,], ST = cov3) -
        ...
        (p_rho(rho=(cov3[upper.tri(cov3)][k])/(cor_to_cov3[k]), .mu = met_mu[i,], ST = cov3) -
  ...
  list(met_mu, mat_sig^2, mat_rho, a_sig, a_rho)
}
